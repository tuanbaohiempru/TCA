...
export enum LiabilityType {
  ...
  PERSONAL_LOAN = 'Vay tiêu dùng/Khác'
}

export interface FinancialAsset {
  id: string;
  type: AssetType;
  name: string; // e.g. "Nhà Quận 7"
  value: number;
  growthRate?: number; // % tăng trưởng dự kiến
}

export interface FinancialLiability {
  id: string;
  type: LiabilityType;
  name: string; // e.g. "Vay Techcombank"
  amount: number;
  interestRate?: number; // % lãi suất vay
  endDate?: string; // Ngày tất toán dự kiến
}
...
export enum AppointmentResult {
  SIGNED = 'Đã chốt ký',
  INTERESTED = 'Quan tâm / Chăm sóc tiếp',
  RESCHEDULE = 'Hẹn lại / Gửi bảng minh họa',
  BUSY = 'Khách bận / Không liên lạc được',
  REJECTED = 'Từ chối / Không nhu cầu',
  DONE = 'Đã xong'
}
