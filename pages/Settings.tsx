...
import { AgentProfile, SalesTargets, Product } from '../types';
import { uploadFile } from '../services/storage';
import { CurrencyInput } from '../components/Shared';
import { clearFirebaseConfig } from '../services/firebaseConfig';
import ProductsPage from './Products'; // Reuse the existing Product Manager
...
const SettingsPage: React.FC<SettingsPageProps> = ({ 
    profile, onSave, isDarkMode, toggleDarkMode,
             ...
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-200 dark:border-gray-800 pb-4">
                 ...
                 <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
                     <button 
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition flex items-center gap-2 ${activeTab === 'profile' ? 'bg-white dark:bg-pru-card shadow text-pru-red' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                     >
                         <i className="fas fa-user-cog"></i> Hồ sơ TVV
                     </button>
                     <button 
                        onClick={() => setActiveTab('products')}
                        className={`px-4 py-2 rounded-lg text-sm font-bold transition flex items-center gap-2 ${activeTab === 'products' ? 'bg-white dark:bg-pru-card shadow text-pru-red' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                     >
             ...
             <div className="flex-1 overflow-y-auto pb-4 custom-scrollbar">
                ...
                {activeTab === 'products' && (
                    <div className="animate-fade-in">
                        {/* Embed ProductsPage here, passing CRUD functions */}
                        <ProductsPage 
                            products={products}
                            onAdd={onAddProduct}
